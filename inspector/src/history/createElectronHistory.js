 ...
 */
const createElectronHistory = (props = {}) => {
  const { getUserConfirmation, keyLength = 6 } = props;

  const storeEntries = store.get("history.entries");
  const storeIndex = store.get("history.index");
  const initialEntries = storeEntries === undefined ? ["/"] : storeEntries;
  const initialIndex = storeIndex === undefined ? 0 : storeIndex;
  ...
  const setState = nextState => {
    ...
    store.set("history.index", history.index);
    store.set("history.entries", history.entries);
...
export default createElectronHistory;
